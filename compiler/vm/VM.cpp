...
namespace vayu {
    ...
    void VM::run(std::shared_ptr<Chunk> entryChunk) {
        ...
        frames_.back().nameCache.assign(
            frames_.back().chunk->names.size(), nullptr);
        runLoop(0);
    }
    ...
    //
    // Phase 25.0a: the try/catch wraps the *inner* dispatch loop, not each
    // opcode.  MSVC enters a TEB frame per try-block execution; with the
    // per-instruction try, hot loops paid 10-20 extra instructions per
    // opcode dispatched.  Hoisting makes it once-per-runLoop-entry, with
    // a `for(;;)` re-entering the try only after an unwind.
    // ===========================================================================
            ...
            try {
                while (frames_.size() > stopAtFrameCount) {
                    ...
                    case OpCode::LOAD: {
                        ...
                        Value* cached = frame->nameCache[idx];
                        if (cached) { stack_.push_back(*cached); break; }
                        Value* slot = frame->env->lookup(name);
                        if (slot) {
                            frame->nameCache[idx] = slot;
                    ...
                    case OpCode::STORE: {
                        ...
                        Value* cached = frame->nameCache[idx];
                        if (cached) { *cached = std::move(v); break; }
                        if (!frame->env->assign(name, v))
                            runtimeError("name '" + name + "' is not defined");
                        // Do not attempt to cache here.  The only path that
                        // populates the cache is LOAD, which fires before
                        // STORE in every real assignment pattern.
                        break;
                    }
                    case OpCode::DEFINE: {
                        int idx = (int(code[ip]) << 8) | int(code[ip + 1]); ip += 2;
                        ...
                        /* A DEFINE on a name that was previously cached can
                           change which scope the name binds to.  Invalidate
                           this entry so the next LOAD re-resolves. */
                        frame->nameCache[idx] = nullptr;
                        break;
                    }
    ...
    void VM::callVMFunction(const std::shared_ptr<Callable>& fn,
        ...
        frames_.back().nameCache.assign(fn->chunk->names.size(), nullptr);
    }
    ...
    Value VM::createVMGenerator(const std::shared_ptr<Callable>& fn,
        ...
        gen->worker = std::thread([gen, fnChunk, fnParams, closure,
            globals, args, interp]() {
                ...
                vm.frames_.back().nameCache.assign(fnChunk->names.size(), nullptr);
